...
-- 상품사진
DROP TABLE IF EXISTS MY_SCHEMA.PHOT RESTRICT;

-- 나의관심
DROP TABLE IF EXISTS MY_SCHEMA.INTER RESTRICT;

-- 입찰기록
DROP TABLE IF EXISTS MY_SCHEMA.BDHS RESTRICT;

-- 상품
DROP TABLE IF EXISTS MY_SCHEMA.ITEM RESTRICT;

-- 채팅
DROP TABLE IF EXISTS MY_SCHEMA.CHAT RESTRICT;

-- 낙찰품결제
DROP TABLE IF EXISTS MY_SCHEMA.PAYMT RESTRICT;
...
ALTER TABLE MY_SCHEMA.PAYMT
  ADD CONSTRAINT FK_BDHS_TO_PAYMT -- 입찰기록 -> 낙찰품결제
    ...
    REFERENCES MY_SCHEMA.BDHS ( -- 입찰기록
      HSNO -- 입찰기록일련번호
    );